...
static u8 ChooseWildMonLevel(const struct WildPokemon* wildPokemon)
{
	...
	if(VarGet(VAR_WILD_LEVEL_SCALING) == 1)
	{
		min = max = GetLowestMonLevel(gPlayerParty);

		#ifdef FLAG_HARD_LEVEL_CAP
		u8 levelCap;
		if ((FlagGet(FLAG_HARD_LEVEL_CAP) || VarGet(VAR_GAME_DIFFICULTY) != OPTIONS_EASY_DIFFICULTY) && max >= (levelCap = GetCurrentLevelCap()))
			min = max = levelCap;
		#endif
	}
	else
...
bool8 StandardWildEncounter(const u32 currMetaTileBehavior, const u16 previousMetaTileBehavior)
{
	...
	if (FlagGet(FLAG_NO_RANDOM_WILD_ENCOUNTERS) || VarGet(VAR_NO_RANDOM_WILD_ENCOUNTERS) == 1)
		return FALSE;
	#endif
...
void sp118_StartRaidBattle(void)
{
	...
	if ((FlagGet(FLAG_DOUBLE_WILD_BATTLE) || VarGet(VAR_DOUBLE_BATTLE) == 1) && ViableMonCount(gPlayerParty) >= 2)
		gBattleTypeFlags |= BATTLE_TYPE_DOUBLE;
